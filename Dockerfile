...
ARG CACHEBUST=0
RUN yum clean all
RUN yum -y install --disablerepo=* --enablerepo=metwork_${BRANCH} -q list metwork-mfext-layer-python3_scientific metwork-mfext-layer-python3_devtools 2>/dev/null |sort |md5sum |awk '{print $1;}' > /tmp/yum_cache
